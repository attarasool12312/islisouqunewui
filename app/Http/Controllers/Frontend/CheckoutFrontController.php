...
class CheckoutFrontController extends Controller
{
    ...
    public function LoadThanks($order_id)
    {
        $display_order='ORD-'.$order_id;
        return view('frontend.thanks',compact('order_id'));
    }
    public function LoadMakeOrder(Request $request)
    {
        ...
        if(isset(Auth::user()->id)){
            ...
            $customer_id_test = $request->input('customer_id');
            if ($customer_id !==$customer_id_test){
                $res['msgType'] = 'error';
                $res['msg'] = array('oneError' => array(__('Oops! Error occured')));
		...
		if($index>0){
			...
			$res['msgType'] = 'success';
			$res['msg'] = __('Your order is successfully, order number is');
			$res['intent'] = $intent;
            $res['order_no']=$order_no;
			return response()->json($res);
//            return Redirect::route('frontend.thanks/'.$order_no);
           // return Redirect::route('frontend.thanks', [$order_no])->with('message', 'Your order is successfully');
		}else{
			$res['msgType'] = 'error';
