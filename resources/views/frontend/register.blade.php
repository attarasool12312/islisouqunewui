					...
					<div class="register">
						...
						<form class="form" method="POST" action="{{ route('frontend.customer-register') }}">
							...
							<div class="form-group">
								<input name="name" type="text" class="form-control alphaonly @error('name') is-invalid @enderror" placeholder="{{ __('Name') }}" value="{{ old('name') }}" required />
                                @if ($errors->has('name'))
                                <span class="text-danger">{{ $errors->first('name') }}</span>
                                @endif
							</div>
							<div class="form-group">
								<input name="email" type="email" class="form-control emailalphanumericonly @error('email') is-invalid @enderror" placeholder="{{ __('Email Address') }}" value="{{ old('email') }}" required />
                                @if ($errors->has('email'))
							...
							<div class="form-group">
                                ...
                                <span class="text-danger">{{ $errors->first('password') }}</span>
									<span class="text-danger">password to have at least 10 characters,1 uppercase, 1 lowercase and a number</span>
                                @endif
							</div>
